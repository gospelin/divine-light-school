            ...
            <form action="{{ isset($class) ? route('admin.classes.update', $class) : route('admin.classes.store') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $class->name ?? '') }}"
                            class="w-full border rounded-lg px-4 py-3" placeholder="e.g. JSS 1" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Group/Arm (Optional)</label>
                        <input type="text" name="group" value="{{ old('group', $class->group ?? '') }}"
                            class="w-full border rounded-lg px-4 py-3" placeholder="A, B, C..." maxlength="1">
                        <p class="text-xs text-gray-500 mt-1">Leave empty for no group (e.g. just "JSS 1")</p>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Sort Order</label>
                        <input type="number" name="order" value="{{ old('order', $class->order ?? 0) }}"
                ...
                <div class="mt-8 flex gap-4">
                    ...
                    <a href="{{ route('admin.classes.index') }}"
                        class="px-8 py-3 border rounded-lg hover:bg-gray-50">Cancel</a>
                </div>
            </form>
        </div>
    </div>
