...
@section('content')
    <section class="py-16 sm:py-24 bg-white min-h-screen">
        <div class="container max-w-7xl mx-auto px-6">
            <h1 class="text-4xl font-bold mb-12 text-[#083873]">Edit Event</h1>

            <form action="{{ route('admin.events.update', $event) }}" method="POST" class="max-w-4xl">
                @csrf @method('PUT')
                <div class="bg-gray-50 rounded-2xl shadow-xl p-10 space-y-8">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" required
                            class="w-full px-6 py-4 rounded-lg border focus:border-[#083873]">{{ old('description', $event->description) }}</textarea>
                        @error('description')
                            <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        <input type="checkbox" name="published" id="published" value="1" {{ old('published', $event->published) ? 'checked' : '' }}
